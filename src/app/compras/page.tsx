...
export default function ComprasPage() {
  ...
  const statusBadge = (status: string) => {
    const config: Record<string, { bg: string; text: string; label: string; border: string }> = {
      ...
      aprovado: { bg: "bg-emerald-500/10", text: "text-emerald-500", border: "border-emerald-500/30", label: "Aprovado / Aguardando" },
      parcial: { bg: "bg-amber-500/10", text: "text-amber-500", border: "border-amber-500/30", label: "Recebido Parcial" },
      recebido: { bg: "bg-teal-500/10", text: "text-teal-500", border: "border-teal-500/30", label: "Totalmente Recebido" },
    ...
    return (
      <span className={cn("px-2.5 py-1 text-xs font-semibold rounded-full border", c.bg, c.text, c.border)}>
        {c.label}
      </span>
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface p-4 rounded-xl border border-border-main shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-1.5 bg-surface border border-border-main text-text-main rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary"
          />
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="bg-surface border border-border-main text-xs rounded-lg px-3 py-1.5 text-text-main focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
          >
      ...
      <div className="space-y-4">
        {pedidosFiltrados.map((pedido) => (
          <div
            key={pedido.id}
            className="bg-surface rounded-xl border border-border-main shadow-sm overflow-hidden hover:border-brand-primary/40 transition-all"
          >
            {/* Faixa Superior / Cabeçalho do Card (Theming Inteligente) */}
            <div className="p-4 bg-surface-hover/80 border-b border-border-main flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div className="flex items-center gap-3">
                <span className="text-sm font-bold text-text-main bg-surface px-2.5 py-1 rounded-lg border border-border-main shadow-xs">
            ...
            <div className="p-4 space-y-3">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                <div>
                  <span className="text-text-dim block text-[11px]">Data de Emissão</span>
                  <span className="font-medium text-text-main">{formatDate(pedido.data_emissao || pedido.created_at)}</span>
                </div>
                <div>
                  <span className="text-text-dim block text-[11px]">Previsão de Entrega</span>
                  <span className="font-medium text-text-main">
                    {formatDate(pedido.previsao_entrega || pedido.data_prevista_entrega || "")}
                  </span>
                </div>
                <div>
                  <span className="text-text-dim block text-[11px]">Condição Pagto</span>
                  <span className="font-medium text-text-main">{pedido.condicao_pagamento || "-"}</span>
                </div>
                <div>
                  <span className="text-text-dim block text-[11px]">Total de Itens</span>
                  <span className="font-medium text-text-main">{pedido.itens?.length || 0} item(ns)</span>
                </div>
              </div>
              ...
              {pedido.status !== "recebido" && pedido.status !== "cancelado" && (
                <div className="flex items-center justify-end gap-2 pt-2">
                  <Link
                    ...
                    className="px-3 py-1.5 bg-brand-primary/10 hover:bg-brand-primary/20 text-brand-primary border border-brand-primary/30 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition-colors"
                  >
          ...
          <div className="bg-surface rounded-2xl max-w-2xl w-full p-6 shadow-2xl border border-border-main space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSalvarPedido} className="space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2 bg-surface border border-border-main text-text-main rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
                    required
                  >
                    {fornecedores.map((f) => (
              ...
              <div className="space-y-2 pt-2 border-t border-border-subtle">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-brand-primary hover:underline font-semibold text-xs flex items-center gap-1"
                  >
                    + Adicionar Insumo
                  </button>
                </div>
                ...
                {itens.map((it, idx) => (
                  <div key={idx} className="p-3 bg-surface-hover rounded-lg border border-border-main grid grid-cols-1 sm:grid-cols-4 gap-2 items-end">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-border-subtle">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-brand-primary hover:bg-brand-hover text-brand-foreground rounded-lg font-semibold shadow-sm"
                >
