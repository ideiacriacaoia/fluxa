...
export default function PersonalizacaoPage() {
  ...
  const [modalEtapa, setModalEtapa] = useState<1 | 2>(1);
  const [osDetalhes, setOsDetalhes] = useState<any | null>(ordensPersonalizacao[0] || null);
  const [abaDetalhes, setAbaDetalhes] = useState<"custos" | "historico">("custos");
  ...
  const getStatusBadge = (st: StatusOSPersonalizacao) => {
    const map: Record<StatusOSPersonalizacao, { label: string; bg: string; text: string }> = {
      ...
      entregue: { label: "Entregue", bg: "bg-slate-500/15", text: "text-slate-400" },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          onClick={handleAbrirNovaOS}
          className="flex items-center gap-2 px-3.5 py-2 bg-brand-primary hover:bg-brand-hover text-brand-foreground text-xs font-semibold rounded-lg shadow-sm transition-colors self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-4 bg-surface rounded-xl border border-border-main shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-text-muted">Margem Média de OS</span>
      ...
      {/* Grid Principal: Lista de OS e Painel Lateral com Sub-Abas */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Lista de Ordens de Serviço */}
        <div className="lg:col-span-7 space-y-4">
          <div className="p-3.5 bg-surface rounded-xl border border-border-main shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="space-y-3">
            {ordensFiltradas.length === 0 ? (
              <div className="p-8 text-center bg-surface rounded-xl border border-border-main text-xs text-text-muted">
                Nenhuma Ordem de Serviço encontrada com os filtros selecionados.
              </div>
            ) : (
              ordensFiltradas.map((os) => {
                const isSelected = os.id === osDetalhes?.id;
                const isExpandido = osExpandidaId === os.id;
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-[11px] text-text-muted mt-1 font-mono">
                          CNPJ: {formatCNPJ(os.cliente_documento)}
                        </p>
                      </div>
                    ...
                    <div className="mt-3 p-2.5 bg-background rounded-lg border border-border-subtle flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                      <div className="flex items-center gap-2">
                        <Shirt className="w-4 h-4 text-brand-primary flex-shrink-0" />
                        <div>
                    ...
                    {/* Tags de Componentes & Botão Expansível */}
                    <div className="flex items-center justify-between gap-2 mt-2.5">
                      <div className="flex flex-wrap items-center gap-1.5">
                        {os.componentes.slice(0, 2).map((c, idx) => (
                          <span
                            key={idx}
                        ...
                        {os.componentes.length > 2 && (
                          <span className="text-[10px] px-1.5 py-0.5 bg-brand-primary/10 text-brand-primary font-semibold rounded">
                            +{os.componentes.length - 2} itens
                          </span>
                        )}
                      </div>
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setOsExpandidaId(isExpandido ? null : os.id);
                        }}
                        className="text-[11px] font-medium text-brand-primary hover:underline flex items-center gap-1 flex-shrink-0"
                      >
                        {isExpandido ? "Recolher" : "Ver Grade"}
                        <ChevronDown className={`w-3 h-3 transition-transform ${isExpandido ? "rotate-180" : ""}`} />
                      </button>
                    </div>

                    {/* Painel Expansível de Grade & Componentes */}
                    {isExpandido && (
                      <div className="mt-3 pt-3 border-t border-border-subtle space-y-2 text-xs">
                        <div className="grid grid-cols-4 gap-2 text-center p-2 bg-surface-hover rounded-lg">
                          <div>
                            <span className="text-[10px] text-text-dim block">P</span>
                            <strong className="text-text-main">{os.peca_base_tamanho_grade.P}</strong>
                          </div>
                          <div>
                            <span className="text-[10px] text-text-dim block">M</span>
                            <strong className="text-text-main">{os.peca_base_tamanho_grade.M}</strong>
                          </div>
                          <div>
                            <span className="text-[10px] text-text-dim block">G</span>
                            <strong className="text-text-main">{os.peca_base_tamanho_grade.G}</strong>
                          </div>
                          <div>
                            <span className="text-[10px] text-text-dim block">GG</span>
                            <strong className="text-text-main">{os.peca_base_tamanho_grade.GG}</strong>
                          </div>
                        </div>

                        {os.observacoes && (
                          <p className="text-[11px] text-text-muted italic bg-background p-2 rounded border border-border-subtle">
                            Obs: {os.observacoes}
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-text-dim mt-2.5 pt-2 border-t border-border-subtle">
        ...
        {/* Detalhes da OS com Sub-Abas Contextuais */}
        {osDetalhes && (
          <div className="lg:col-span-5 space-y-4 lg:sticky lg:top-4">
            <div className="p-4 bg-surface rounded-xl border border-border-main shadow-xs space-y-4">
              {/* Header do Card */}
              ...
              <div className="p-3 bg-surface-hover rounded-xl border border-border-subtle space-y-2">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  <button
                    ...
                    className="px-2.5 py-1.5 bg-surface hover:bg-border-main text-text-main text-[11px] font-semibold rounded border border-border-main transition-colors"
                  >
                    ▶ Em Produção
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="px-2.5 py-1.5 bg-status-success hover:bg-status-success/90 text-white text-[11px] font-semibold rounded transition-colors"
                  >
                    ✓ Finalizar
                  </button>
                </div>
              ...
              <div className="flex border-b border-border-subtle">
                ...
                <button
                  type="button"
                  onClick={() => setAbaDetalhes("historico")}
                  className={`flex-1 py-2 text-xs font-semibold text-center border-b-2 transition-colors flex items-center justify-center gap-1.5 ${
              ...
              {abaDetalhes === "custos" && (
                <div className="space-y-3 text-xs animate-in fade-in">
                  <div className="p-3 bg-background rounded-lg border border-border-subtle space-y-1.5">
                    ...
                    <div className="border-t border-border-subtle pt-1.5 flex justify-between font-bold text-text-main">
                  ...
                  {/* Resumo Comercial */}
                  <div className="p-3 bg-brand-primary/10 rounded-lg border border-brand-primary/20 space-y-1.5 text-xs">
                    <div className="flex justify-between">
                      <span className="text-text-muted">Custo Total ({osDetalhes.quantidade_total_pecas} un):</span>
                    ...
                    <div className="flex justify-between font-bold text-sm border-t border-brand-primary/30 pt-1.5 text-text-main">
              ...
              {/* Aba 2: Histórico & Rastreabilidade */}
              {abaDetalhes === "historico" && (
      ...
      {/* MODAL: Nova OS de Personalização — Dividido em 2 Etapas */}
      {modalNovaOS && (
            ...
            <div className="flex items-center justify-between border-b border-border-subtle pb-3">
              <div>
                ...
                <span className="text-[11px] text-text-muted">
                  Etapa {modalEtapa} de 2: {modalEtapa === 1 ? "Cliente & Peça Base" : "Componentes Extras & Margem"}
                </span>
              </div>
              <button onClick={() => setModalNovaOS(false)} className="text-text-dim hover:text-text-main p-1">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg text-center transition-all ${
                  modalEtapa === 1
                    ? "bg-brand-primary text-brand-foreground shadow-xs"
                    : "bg-surface-hover text-text-muted hover:text-text-main"
                }`}
              >
                1. Cliente & Peça Base
              ...
              <button
                type="button"
                onClick={() => {
                  if (clienteNome) setModalEtapa(2);
                }}
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg text-center transition-all ${
                  modalEtapa === 2
                    ? "bg-brand-primary text-brand-foreground shadow-xs"
                ...
                <div className="space-y-3 animate-in fade-in">
                  ...
                  <div className="grid grid-cols-3 gap-3 pt-2 border-t border-border-subtle">
                    <div className="col-span-2">
                      <label className="block font-semibold text-text-main mb-1">Produto Base em Estoque</label>
                      <select
                        value={pecaBaseId}
                  ...
                  <div className="flex justify-end gap-2 pt-3 border-t border-border-subtle">
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 bg-brand-primary hover:bg-brand-hover text-brand-foreground font-semibold rounded-lg shadow-sm flex items-center gap-1.5"
                    >
                      Próxima Etapa: Componentes
                ...
                <div className="space-y-3 animate-in fade-in">
                  ...
                  <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                    {componentes.map((c, idx) => (
                      <div key={idx} className="p-2.5 bg-surface-hover rounded-xl border border-border-subtle grid grid-cols-12 gap-2 items-center">
                  ...
                  <div className="p-3 bg-brand-primary/10 rounded-xl border border-brand-primary/30 grid grid-cols-3 gap-3 text-xs">
                    <div>
                      <span className="text-text-muted block text-[10px]">Custo Base + Extras:</span>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-background border border-border-main rounded px-3 py-1.5 text-text-main"
                      />
                    </div>

                    <div>
                      <label className="block font-semibold text-text-main mb-1">Prioridade</label>
                      <select
                        value={prioridade}
